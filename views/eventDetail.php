        ...
        <div class="container mx-auto max-w-2xl">
            <div class="event-card bg-white p-6 rounded-lg shadow-lg">
                <div class="user-info flex items-center space-x-4 mb-6">
                    <span class="user-profile"><div class="rounded-full bg-gray-200 w-12 h-12 flex items-center justify-center overflow-hidden"><img src="/src/image/profile.png" class="w-full h-full object-cover" alt="<?= htmlspecialchars($event['user_name']) ?>'s Profile"></div></span>
                    <div><span class="user-name block font-semibold text-lg"><?= htmlspecialchars($event['user_name']) ?></span><span class="event-topic text-gray-600 text-sm">#<?= htmlspecialchars($event['topic']) ?></span></div>
                </div>
                <?php if (!empty($event['image_url'])): ?><div class="mb-6 rounded-lg overflow-hidden shadow-md"><img src="/<?= htmlspecialchars($event['image_url']) ?>" class="w-full h-auto object-contain max-h-[60vh]" alt="<?= htmlspecialchars($event['title']) ?>"></div><?php endif; ?>
