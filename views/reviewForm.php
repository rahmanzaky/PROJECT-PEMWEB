...
<head>
    ...
    <title id="event-title">Review - GrowTogether</title>
</head>
<body class="bg-gray-100 font-sans text-gray-800 pb-24">
    ...
    <header class="bg-amber-600 text-white p-4 fixed m-auto w-full shadow-md z-20 top-0">
        <div class="container mx-auto flex justify-between items-center">
            <h1 class="text-2xl font-bold text-white"><a href="#" id="event-detail-link" class="mr-2">Event Review</a></h1>
    ...
    <main class="py-8 px-4 mt-16">
        <div class="container mx-auto max-w-lg">
            <div class="bg-white p-6 rounded-lg shadow-md mb-6 border-l-4 border-amber-600" id="event-header">
            
            </div>
            <div id="review-form-section"></div>
            <div class="mt-10 bg-white p-6 rounded-lg shadow-md" id="reviews-section">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            document.getElementById('event-title').textContent = 'Review: ' + event.title + ' - GrowTogether';
            document.getElementById('event-detail-link').textContent = 'Event Review';
            ...
            document.getElementById('event-header').innerHTML = `
                ...
                <h2 class="text-xl font-semibold text-red-500">${event.title}</h2>
                <p class="text-xs text-gray-500">by ${event.user_name}</p>
            `;
            document.getElementById('review-form-section').innerHTML = `
                <form id="review-form" class="bg-white p-6 rounded-lg shadow-md mb-8">
                    <div class="mb-4">
            ...
            document.getElementById('review-form').addEventListener('submit', async (e) => {
                ...
                try {
                    const res = await fetch('http://localhost:8080/events/' + eventId + '/reviews', {
                        method: 'POST',
                        headers: { 
                    ...
                    e.target.reset();
                    loadReviews();
                    showPopup('Your review has been submitted!');
                } catch (err) {
                    ...
                    if (err.message && err.message.toLowerCase().includes('foreign key constraint fails')) {
                        ...
                        showPopup(err.message || 'Failed to submit your review. Please try again.', true);
                    }
                }
            });
            async function loadReviews() {
                const reviewsRes = await fetch('http://localhost:8080/events/' + eventId + '/reviews');
                const reviews = await reviewsRes.json();
                const reviewsSection = document.getElementById('reviews-section');
                if (!reviews.length) {
                    reviewsSection.innerHTML = '<p class="text-gray-500">No reviews yet for this event. Be the first!</p>';
                } else {
                    reviewsSection.innerHTML = `<h3 class="text-xl font-semibold mb-6 border-b border-amber-600/60 pb-3 text-gray-700">Event Reviews (${reviews.length})</h3>` +
                        reviews.map(review => `
                        <div class="review-item mb-6 pb-4 border-b border-amber-600/60 last:border-b-0 last:mb-0 last:pb-0">
                            <div class="flex items-start space-x-3">
                                <div class="flex-shrink-0"><div class="rounded-full bg-gray-200 w-10 h-10 flex items-center justify-center overflow-hidden"><img src="/src/image/profile.png" class="w-full h-full object-cover" alt="${review.user_name}'s Profile"></div></div>
                                <div class="flex-grow">
                                    <p class="font-semibold text-gray-800">${review.user_name}</p>
                                    <div class="flex items-center mb-1">${[1,2,3,4,5].map(s => `<span class="text-2xl ${s <= review.rating ? 'text-yellow-500' : 'text-gray-300'}">&#9733;</span>`).join('')}<span class="ml-2 text-xs text-gray-500">(${review.rating} star${review.rating == 1 ? '' : 's'})</span></div>
                                    <p class="text-xs text-gray-500 mb-1">Reviewed on: ${new Date(review.created_at).toLocaleString('en-US', { timeZone: 'Asia/Jakarta' })}</p>
                                    ${review.review_text ? `<p class="text-gray-700 whitespace-pre-line mt-2 text-sm">${review.review_text}</p>` : ''}
                                </div>
                            </div>
                        </div>
                    `).join('');
