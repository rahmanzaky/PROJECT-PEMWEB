...
<body class="bg-gray-100 font-sans text-gray-800 pb-24">
    ...
    <header class="bg-amber-600 text-black p-4 fixed m-auto w-full shadow-md z-20 top-0">
        <div class="container mx-auto flex justify-between items-center">
            <h1 class="text-2xl font-bold text-white"><a href="?c=Todos&m=grow" class="hover:text-white">GrowTogether</a></h1>
   ...
   <div id="mobile-menu" class="fixed top-0 right-0 h-full w-72 bg-orange-50 backdrop-blur-lg shadow-xl transform translate-x-full transition-transform duration-300 ease-in-out z-40">
    ...
    <main class="py-4 px-4 mt-20">
        <div class="container mx-auto">
            <form method="POST" action="?c=Todos&m=search" class="flex w-full mb-6">
                <input 
            ...
            <?php if (!empty($events)): ?>
                <?php foreach ($events as $event): ?>
                    <a href="?c=Todos&m=showEvent&id=<?= $event['id'] ?>" class="block event-card-small border border-gray-200 rounded-lg p-4 shadow-lg hover:shadow-xl transition-shadow duration-200 bg-white flex flex-col">
